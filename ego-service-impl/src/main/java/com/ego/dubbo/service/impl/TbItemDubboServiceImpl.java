...
public class TbItemDubboServiceImpl implements TbItemDubboService {
	...
	@Override
	public int insTbItemDesc(TbItem tbItem, TbItemDesc desc,TbItemParamItem param) throws Exception {
		int index =0;
		try {
			index= tbItemMapper.insertSelective(tbItem);
			index+= tbItemDescMapper.insertSelective(desc);
			index+= tbItemParamItemMapper.insertSelective(param);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(index==3){
	...
	@Override
	public TbItem selByPrimaryKey(long id) {
